...
const UserProfile = () => {
      ...
      try {
        ...
        setEditData({
          name:
            ...
            profileData.store_settings?.business_name ||
            "User",
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      if (editData.name)
        formData.append("personal_info[fullname]", editData.name);
      if (editData.email)
        formData.append("personal_info[email]", editData.email);
      if (editData.phone)
        formData.append("personal_info[phone]", editData.phone);

      // Add store settings fields
      if (editData.address)
        formData.append("store_settings[address]", editData.address);
      if (editData.bio)
        formData.append("store_settings[description]", editData.bio);
      ...
      <div className="min-h-[90vh] flex flex-col items-center justify-center px-4 bg-gray-50/50">
        <div className="max-w-md w-full bg-white rounded-3xl p-8 md:p-12 shadow-xl shadow-gray-200/50 text-center border border-gray-100">
          ...
          <p className="text-gray-500 text-sm leading-relaxed mb-8">
            We couldn't retrieve your profile data. You may have been logged out
  ...
  return (
    <div className="min-h-screen bg-background pb-10">
      ...
      <div className="relative">
        <div
          className="h-40 md:h-64 w-full bg-center bg-cover"
      ...
      <div className="max-w-7xl mx-auto px-4 mt-16 md:mt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <aside className="md:col-span-4 space-y-6">
            ...
            <div className="p-6 border rounded-2xl bg-white shadow-sm">
              <h3 className="text-xs font-black uppercase tracking-widest mb-4 text-gray-400">
                Contact Details
              </h3>
            ...
            <div className="p-6 border rounded-2xl bg-white shadow-sm">
              ...
              >
                <Icon
                  name={
                    store_settings?.is_published ? "CheckCircle" : "AlertCircle"
                  }
                  size={16}
                />
                <span className="text-xs font-bold">
          ...
          <main className="md:col-span-8">
            <div className="mb-20 border rounded-3xl bg-white shadow-sm overflow-hidden min-h-[500px]">
              <div className="flex border-b bg-gray-50/50">
                {["saved", "settings"].map((tab) => (
                  <button
                    ...
                    className={`flex-1 py-5 text-xs font-black tracking-widest transition-all ${
                      activeTab === tab
              ...
              <div className="p-8">
                ...
                {activeTab === "settings" && <Settings isReadOnly={true} />}
                {/* {activeTab === "history" && (
